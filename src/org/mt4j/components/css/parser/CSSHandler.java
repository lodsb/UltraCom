...
public class CSSHandler implements DocumentHandler {
            ...
            switch (prop) {
                ...
                case FONTSIZE:
                    ...
                    if (CSSKeywords.isMeasuringUnit(parameter)) {
                        if (currentFont == null)
                            currentFont = new CSSFont((int) (parseMeasuringUnit(parameter, defaultFontSize) * (72f / 100f)));
                        else
                            currentFont.setFontsize((int) (parseMeasuringUnit(parameter, defaultFontSize) * (72f / 100f)));
    ...
    private BackgroundRepeat handleBackgroundRepeat(LexicalUnit value) {
        ...
        if (value.getLexicalUnitType() == LexicalUnit.SAC_IDENT) {
            if (value.getStringValue().replaceAll(" ", "").equalsIgnoreCase("REPEAT-X"))
                return BackgroundRepeat.XREPEAT;
            if (value.getStringValue().replaceAll(" ", "").equalsIgnoreCase("REPEAT-Y"))
                return BackgroundRepeat.YREPEAT;
    ...
    private void handleFontWeight(LexicalUnit value) {
        ...
        switch (value.getLexicalUnitType()) {
            ...
            case LexicalUnit.SAC_STRING_VALUE:
                if (value.getStringValue().toUpperCase().contains("BOLD")) {
                    currentFont.setWeight(CSSFontWeight.BOLD);
                    break;
                }
                if (value.getStringValue().toUpperCase().contains("LIGHT")) {
                    currentFont.setWeight(CSSFontWeight.LIGHT);
                    break;
                }
                currentFont.setWeight(CSSFontWeight.NORMAL);
                break;
            case LexicalUnit.SAC_INTEGER:
                if (value.getIntegerValue() < 400) {
                    currentFont.setWeight(CSSFontWeight.LIGHT);
                    break;
                }
                if (value.getIntegerValue() > 600) {
                    currentFont.setWeight(CSSFontWeight.BOLD);
                    break;
                }
                currentFont.setWeight(CSSFontWeight.NORMAL);
    ...
    private void handleFontFamily(LexicalUnit value) {
        ...
        if (value.getLexicalUnitType() == LexicalUnit.SAC_IDENT || value.getLexicalUnitType() == LexicalUnit.SAC_STRING_VALUE) {
            ...
            if (value.getStringValue().toUpperCase().contains("MONO")) {
                currentFont.setFamily(CSSFontFamily.MONO);
                return;
            }
            if (value.getStringValue().toUpperCase().contains("SANS")) {
                currentFont.setFamily(CSSFontFamily.SANS);
                return;
            }
            if (value.getStringValue().toUpperCase().contains("SERIF")) {
                currentFont.setFamily(CSSFontFamily.SERIF);
                return;
            }
    ...
    private void handleFontStyle(LexicalUnit value) {
        ...
        if (value.getLexicalUnitType() == LexicalUnit.SAC_IDENT || value.getLexicalUnitType() == LexicalUnit.SAC_STRING_VALUE) {
            if (value.getStringValue().toUpperCase().contains("ITALIC")) {
                currentFont.setStyle(CSSFontStyle.ITALIC);
                return;
            }
            if (value.getStringValue().toUpperCase().contains("OBLIQUE")) {
                currentFont.setStyle(CSSFontStyle.OBLIQUE);
                return;
            }
        }
    }
