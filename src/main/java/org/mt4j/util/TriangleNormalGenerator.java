...
//						logger.debug("vdP0 is empty -> adding first vertex, texture tuple");
                    ...
                    if (myVertexData.equalsVertex(newVertex)) {
                        ...
                        if (duplicateIndex != -1) { //Es gibt schon ein duplicate mit gleichen tex coords wie dieses hier, adde bei duplicateIndex
                            ...
                            //wenn wir orginal hinzufï¿œgen wird auch allen duplicates neighbor face zugefï¿œgt.
                            myVertexData.addNeighborFace(myFace);
                            ...
                            //Neuen Vertex machen, an original vertices list anhï¿œngen, neuen index in face auf diesen setzen, face adden
                            ...
                            //Adde face zu orginal face -> damit wird auch duplicates und dem neuen hinzugefï¿œgt,
...
//					logger.debug("Calcing v normal of face: " + myFace.index + " P2");
        ...
        //Leider nï¿œtig? da manche models newVertices enthalten, die in den faces garnicht referenziert werden, quasi tote verts
...
//			float creaseAngleRad = 
        ...
        if (creaseAngleRad != 0.0) {
            ...
            //wenn smooth -> als smooth neighbor des face pointers hinzufï¿œgen
...
//			}
        ...
        //Jetzt fï¿œgen wir den VertexDatas, die die vertices representieren diese vertex normalen hinzu.
        //Wenn in mehreren faces auf den gleichen vertex gezeigt wird, aber in dem face fï¿œr diesen vertex eine
        //andere normale berechnet wurde (weil face mit anderen smooth ist) mï¿œssen wir evtl das Vertex(data) duplizieren
        //und diesem die andere normale hinzufï¿œgen
        for (int j = 0; j < faces.size(); j++) {
            MyFace currentFace = faces.get(j);
                ...
                if (currentFacesCurrentVDNormal == null) {
                    ...
                    if (currentFacesCurrentVDNormal.equalsVectorWithTolerance(vertexNormalsCurrentFace[i], ToolsMath.ZERO_TOLERANCE)) {
                        ...
                        if (duplicateIndexOfSameVertDiffNormal != -1) { //Es gibt schon ein duplicate mit gleichen tex coords wie dieses hier, adde bei duplicateIndex
                            ...
                            //Neuen Vertex machen, an original vertices list anhï¿œngen, neuen index in face auf diesen setzen
                            VertexData newVertexData = new VertexData();
...
//			}
        ...
        public void addNeighborFace(MyFace face) {
            if (!faces.contains(face))
                faces.add(face);
            //Auch duplicates face hinzufï¿œgen, da sie ja den gleichen vertex sharen
            //und fï¿œr die normalenberechnung alle neighborfaces brauchen
        ...
        public void addNeighborFaces(ArrayList<MyFace> addFaces) {
            ...
            //Auch duplicates faces hinzufï¿œgen, da sie ja den gleichen vertex sharen
            //und fï¿œr die normalenberechnung alle neighborfaces brauchen
            for (VertexData vdDuplicate : this.getDuplicateVertexWithDiffTexCoordsList()) {
