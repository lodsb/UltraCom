        ...
        if (locked.contains(c)) {
            ...
            if (free.size() > 0) {             //check if there are other cursors on the component, we could use
                ...
                if (this.canLock(otherCursor)
                        &&
                        Vector3D.distance2D(buttonDownScreenPos, otherCursor.getPosition()) <= this.maxFingerUpDist) {     //Check if we have the priority to use this other cursor and if cursor is in range
