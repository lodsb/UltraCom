...
public class MTComponent extends BaseComponent implements IMTComponent3D, IGestureEventListener {
    ...
    static ConcurrentLinkedQueue<Object> deferQueue = new ConcurrentLinkedQueue<Object>();


    // reakt/Scala properties
    private _genSetterGetterHelper<Vector3D> _glblPosHlpr =
            new _genSetterGetterHelper(ScalaPropertyBindings.setPositionGlobal(this),new Vector3D(0f, 0f, 0f));
    public final Property<Vector3D> globalPosition;
    private _genSetterGetterHelper<Vector3D> _relPosParHlpr =
            new _genSetterGetterHelper(ScalaPropertyBindings.setPositionRelParent(this),new Vector3D(0f, 0f, 0f));
    public final Property<Vector3D> relativePositionToParent;
    public final Property<Rotation> localRotation;
    private _genSetterGetterHelper<Rotation> _localRot =
            new _genSetterGetterHelper(ScalaPropertyBindings.setRotLocal(this),new Rotation(Vector3D.ZERO_VECTOR, 0f, 0f, 0f));

    public final Property<Rotation> relativeRotationToParent;
    private _genSetterGetterHelper<Rotation> _relRot =
            new _genSetterGetterHelper(ScalaPropertyBindings.setRotRel(this),new Rotation(Vector3D.ZERO_VECTOR, 0f, 0f, 0f));
    private _genSetterGetterHelper<Rotation> _globalRot =
            new _genSetterGetterHelper(ScalaPropertyBindings.setRotGlobal(this),new Rotation(Vector3D.ZERO_VECTOR, 0f, 0f, 0f));

    public final Property<Rotation> globalRotation;
    // since translations are currently not threadsave anyway, here a local var. is used to inidicate
    // triggering Property Signals if the object is changed
    private boolean triggerPropertyUpdate = true;
...
//			this.setLocalInverseMatrixInternal(this.getLocalInverseMatrix().multLocal(ms[1]));
    ...
    public  void rotate3DRelativeToParent(Rotation rotation) {
        this.triggerPropertyUpdate = false;

        Vector3D vec = rotation.rotationPos();
        float degreeX = rotation.degreeX();
        float degreeY = rotation.degreeY();
        float degreeZ = rotation.degreeZ();

        this.rotateX(vec, degreeX, TransformSpace.RELATIVE_TO_PARENT);
        this.rotateY(vec, degreeY, TransformSpace.RELATIVE_TO_PARENT);
        this.rotateZ(vec, degreeZ, TransformSpace.RELATIVE_TO_PARENT);

        this.triggerPropertyUpdate = true;
    }


    public void rotate3DLocal(Rotation rotation) {
        this.triggerPropertyUpdate = false;
        ...
        //vec = MTComponent.getLocalVecToParentRelativeSpace(this, vec);
        this.rotateX(vec, degreeX, TransformSpace.LOCAL);
        this.rotateY(vec, degreeY, TransformSpace.LOCAL);
        this.rotateZ(vec, degreeZ, TransformSpace.LOCAL);

        this.triggerPropertyUpdate = true;
    }

    public void rotate3DGlobal(Rotation rotation) {
        this.triggerPropertyUpdate = false;
        ...
        //vec = MTComponent.getGlobalVecToParentRelativeSpace(this, vec);
        this.rotateX(vec, degreeX, TransformSpace.GLOBAL);
        this.rotateY(vec, degreeY, TransformSpace.GLOBAL);
        this.rotateZ(vec, degreeZ, TransformSpace.GLOBAL);
    ...
    public void rotateX(Vector3D rotationPoint, float degree, TransformSpace transformSpace) {
        ...
        Vector3D origRotation = rotationPoint;
...
//        logger.debug("Determinant after orthogonalize: " + getLocalMatrix().determinant());
    ...
    public void setPositionRelativeToParentNoUpdateTrigger(Vector3D pos) {
        this.triggerPropertyUpdate = false;
        this.setPositionRelativeToParent(pos);
        this.triggerPropertyUpdate = true;
    }
    ...
    public void setPositionRelativeToOtherNoUpdateTrigger(MTComponent otherComp,Vector3D pos) {
        this.triggerPropertyUpdate = false;
        this.setPositionRelativeToOther(otherComp,pos);
        this.triggerPropertyUpdate = true;
    }
    ...
    public Vector3D getCenterPointGlobal() {
        Vector3D center = Vector3D.ZERO_VECTOR;
