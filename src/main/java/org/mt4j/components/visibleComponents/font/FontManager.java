...
//		String fontAbsoultePath = System.getProperty("user.dir") + File.separator + "data" + File.separator + "fonts"+  File.separator + fontFileName;
        ...
        if (new File(fontFileName).exists()) {
            fontAbsolutePath = fontFileName;

        } else {
            fontAbsolutePath = MT4jSettings.getInstance().getDefaultFontPath() + fontFileName;
        }


        //Return cached font if there
        IFont font = this.getCachedFont(fontAbsolutePath, fontSize, fillColor, strokeColor, antiAliased);
        if (font != null) {
            return font;
        }
...
//				loadedFont = factoryToUse.createFont(pa, fontAbsoultePath, fontSize, fillColor, strokeColor);
                loadedFont = factoryToUse.createFont(pa, fontAbsolutePath, fontSize, fillColor, strokeColor, antiAliased);
