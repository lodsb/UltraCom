...
import org.mt4j.input.inputProcessors.globalProcessors.*;
...
/**
 * The Class TouchTracer, which is basically a CursorTracer with slightly altered appearance. 
 * A global input processor tracking all AbstractCursorInputEvt events and
 * displays a circle at that position.
 *
 * @author Christopher Ruff
 */
public class TouchTracer extends AbstractGlobalInputProcessor<MTFingerInputEvt> {
     ...
     */
    public TouchTracer(SimpleAbstractScene currentScene, MTApplication mtApp) {
        this(mtApp, currentScene);
    }

    /**
     * Instantiates a new touch tracer.
     *
     * @param mtApp        the mt app
     * @param currentScene the current scene
     */
    public TouchTracer(MTApplication mtApp,SimpleAbstractScene currentScene) {
        super(currentScene);
        this.app = mtApp;
...
//	}
    ...
    protected AbstractShape createDisplayComponent(PApplet applet, Vector3D position) {
        ...
        displayShape.setFillColor(new MTColor(0, 20, 80, 10));
        return displayShape;
    }
