...
object FastForwardButton {

  def apply(app: Application, menu: Menu, center: Vector3D) = {
      new FastForwardButton(app, menu, center)
  }  
...
class FastForwardButton(app: Application, menu: Menu, center: Vector3D) extends Button(app, menu, center) {
  ...
  override def drawComponent(g: PGraphicsOpenGL) = {
    ...
    val (p1x, p1y) = (c1x - segment, c1y - a/2)
    val (p2x, p2y) = (c1x - segment, c1y + a/2)
    val (p3x, p3y) = (c1x + h - segment, c1y)   
    
    //defining an equilateral triangle circumscribed by a circle of radius r centered around (c2x, c2y)
    val (p4x, p4y) = (c2x - segment, c2y - a/2)
    val (p5x, p5y) = (c2x - segment, c2y + a/2)
