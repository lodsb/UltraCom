...
SynthDef("Polywaves") {
  ...
  val w3 = vol * RLPF.ar(Blip.ar(frq3,tone1).madd(amp3*globAmp,0) + noiseGen, line ,0.4) * percussiveEnvelope
  val w4 = vol * RLPF.ar(Blip.ar(frq7,tone1).madd(amp7*globAmp,0) + noiseGen, line ,0.4) * percussiveEnvelope
  val w5 = vol * RLPF.ar(Blip.ar(frq9,tone1).madd(amp9*globAmp,0) + noiseGen, line ,0.4) * percussiveEnvelope
  val w6 = vol * RLPF.ar(Blip.ar(frq11,tone1).madd(amp11*globAmp,0) + noiseGen, line ,0.4) * percussiveEnvelope
  val w7 = vol * RLPF.ar(Blip.ar(frq13,tone1).madd(amp13*globAmp,0) + noiseGen, line ,0.4) * percussiveEnvelope
