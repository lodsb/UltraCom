...
SynthDef("Polywaves") {
  ...
  val (frq1, frq5, frq3, frq7, frq9, frq11, frq13) =
    (z, z*f2(7), z*f2(4), z*f2(10), z*f2(14), z*f2(17), z*f2(21))
  ...
  val w1 = volume * RLPF.ar(Blip.ar(frq1,tone1).madd(amp1*globAmp,0) + noiseGen,line ,0.4) *
    percussiveEnvelope
  val w2 = volume * RLPF.ar(Blip.ar(frq5,tone1).madd(amp5*globAmp,0) + noiseGen, line ,0.4) *
    percussiveEnvelope
  val w3 = volume * RLPF.ar(Blip.ar(frq3,tone1).madd(amp3*globAmp,0) + noiseGen, line ,0.4) *
    percussiveEnvelope
  val w4 = volume * RLPF.ar(Blip.ar(frq7,tone1).madd(amp7*globAmp,0) + noiseGen, line ,0.4) *
    percussiveEnvelope
  val w5 = volume * RLPF.ar(Blip.ar(frq9,tone1).madd(amp9*globAmp,0) + noiseGen, line ,0.4) *
    percussiveEnvelope
  val w6 = volume * RLPF.ar(Blip.ar(frq11,tone1).madd(amp11*globAmp,0) + noiseGen, line ,0.4) *
    percussiveEnvelope
  val w7 = volume * RLPF.ar(Blip.ar(frq13,tone1).madd(amp13*globAmp,0) + noiseGen, line ,0.4) *
    percussiveEnvelope
